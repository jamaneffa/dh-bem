    ...
    if (productosDelUsuario.length > 0) {
        ...
        productosDelUsuario.forEach((producto, index) => {
            ...
            containerNombreInput.appendChild(nombreInput);
                        ...
                        labelSize.textContent = 'Talle:';
                        const talleInput = document.createElement('input');
                        talleInput.type = 'text';
                        talleInput.name = `talle_${index}`;
                        talleInput.value = producto.talle;
                        talleInput.readOnly = true; 
                        const containerTalleInput = document.createElement('div');
                        fieldsetProducto.appendChild(labelSize);
                        fieldsetProducto.appendChild(containerTalleInput);
                        containerTalleInput.appendChild(talleInput);
